...
1. **Baza de date D1** — Workers & Pages → `concurs-api` → Settings → Bindings → Add binding → D1 database. Variable name: `DB`. Dacă nu există încă baza de date, o creezi din Workers & Pages → D1 → Create database, apoi rulezi conținutul din `schema.sql` (D1 → baza ta → Console). Dacă baza există deja de dinainte, rulează doar blocurile noi (`push_subs`, `season_archive`) din `schema.sql`.
